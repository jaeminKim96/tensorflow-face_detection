...
def for_detect_train():
    ...
    while True:
        ...
        if prediction > 0.5:  # 사람일 때
            ...
            try:
                ...
                # resize 오류 발생 시 해당 프레임은 스킵하고 다음으로 넘어가기
                continue

        else:
            label = "Come to Camera"  # 캠 앞에 사람이 없으
    ...
    def getImageLabel(path, face_detector):
        ...
        for img_path in img_paths:
            ...
            if min_size is None:
                min_size = img_numpy.shape

            min_size = (
                min(min_size[0], img_numpy.shape[0]),
                min(min_size[1], img_numpy.shape[1]),
            )

            labels.append(int(os.path.split(img_path)[-1].split(".")[1]))
    ...
    if os.path.exists(traindatapath):
        ...
        recognizer.read(os.path.join(traindatapath, "traindata.yml"))

    else:
        recognizer = cv2.face.EigenFaceRecognizer_create()

    faces, labels = getImageLabel(datasetpath, face_detector)
    ...
    while True:
        ...
        for (x, y, w, h), conf in zip(faces, confidences):
            ...
            if confidence < 55:
                ...
                #     url = "http://3.39.11.36:8080/insert/Visitor"
