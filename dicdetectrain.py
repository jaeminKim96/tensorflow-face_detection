...
class Folder:
    ...
    def __init__(self, name, id):
        self.name = name
        self.id = id
        self.path = os.path.join(os.getcwd(), "dataset", name)
...
folder = Folder.find_folder_by_name(name)
if folder is None:
        ...
        for face in faces:
            ...
            if count <= 100:
                ...
                if face_img is not None:
                    ...
                    # 파일 이름 정리
                    cv2.imwrite(
                        f"{folder.path}/User_{id}_{name}_{count}.jpg",
                        face_img * 255.0,
                    )
                    count += 1
